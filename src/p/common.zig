...
pub const Location = struct {
    ...
    pub fn from_span(src: []const u8, span: Span) @This() {
...
pub fn TreeFormatter(T: type) type {
    return struct {
        ...
        fn has_format(comptime FieldType: type) bool {
            const info = @typeInfo(FieldType);
            switch (info) {
                .pointer => |ptr| {
                    if (ptr.size == .one) {
                        return has_format(ptr.child);
                    }
                    return false;
                },
        ...
        fn print_indent(writer: *Io.Writer, depth: usize) !void {
            for (0..depth) |_| try writer.print(color.SEP, .{});
        }

        fn print_base_value(writer: *Io.Writer, field_name: []const u8, value: anytype, depth: usize) !void {
            ...
            switch (@typeInfo(T)) {
                .@"struct" => |s| inline for (s.fields) |field| switch (@typeInfo(field.type)) {
                    .pointer => |ptr| switch (ptr.size) {
                        .one => {
                            if (comptime has_format(field.type)) {
                        ...
                        .slice => {
                            ...
                            if (ptr.child == u8) {
                                try print_base_value(writer, field.name, slice_val, depth + 1);
                            } else {
                                ...
                                if (is_string_slice) {
                                    ...
                                    for (slice_val) |f| {
                                        if (comptime has_format(@TypeOf(f))) {
                    ...
                    .optional => if (@field(this.data, field.name)) |f| {
                        ...
                        if (comptime mem.find(u8, @typeName(FieldType), "Box")) |_| {
                            if (comptime has_format(@TypeOf(@field(f, "value")))) {
                            ...
                            switch (f) {
                                inline else => |union_val| {
                                    if (comptime has_format(@TypeOf(union_val))) {
                                        try writer.print("{f}", .{union_val.format(depth + 1)});
                                    } else {
                                        try print_base_value(writer, field.name, union_val, depth + 1);
                                    }
                                },
                            }
                        } else {
                            if (comptime has_format(FieldType)) {
                                try writer.print("{f}", .{f.format(depth + 1)});
                            } else {
                                try print_base_value(writer, field.name, f, depth + 1);
                            }
                        }
                    },
                    else => {
                        ...
                        if (comptime mem.find(u8, @typeName(field.type), "Box")) |_| {
                            if (comptime has_format(@TypeOf(@field(field_val, "value")))) {
                            ...
                            if (comptime has_format(field.type)) {
                                ...
                                try print_base_value(writer, field.name, field_val, depth + 1);
                            }
                        }
                    },
                },
                    ...
                    inline else => |f| {
                        if (comptime mem.find(u8, @typeName(@TypeOf(f)), "Box")) |_| {
                            ...
                            if (comptime has_format(@TypeOf(f))) {
                                ...
                                try print_base_value(writer, @tagName(this.data.*), f, depth + 1);
                            }
                        }
                    },
                },
